...
const DriverList = () => {
  ...
  const [reviewTarget, setReviewTarget] = useState<DriverInvite | null>(null);
  ...
  const openReviewModal = (item: DriverInvite) => {
    if (!user) {
      message.warning('请先登录');
      navigate('/login');
      return;
  ...
  const renderPrice = (item: DriverInvite) => {
    if (item.price) {
      return <span className="price-tag">¥{item.price}/人</span>;
    }
    return <span>面议</span>;
  };
  ...
  return (
    <div>
      <Card title="车主邀客列表" style={{ marginBottom: 16 }}>
        <Space wrap>
          <DatePicker
            placeholder="选择出行日期"
